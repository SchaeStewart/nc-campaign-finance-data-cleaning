...
const init = () => {
    if (!filePath || !tableName) {
        console.error('tableName or filepath is invalid. Please provide both as `node bin/importFromCSV my_table path/to/file.csv')
...
(async () => {
  ...
  try {
    ...
    const beforeCount = (await client.query(`select count(*) from ${tableName}`)).rows
    console.log('Count before', beforeCount[0].count)
    await copyFromCSV(filePath, tableName, client);
    const afterCount = (await client.query(`select count(*) from ${tableName}`)).rows
